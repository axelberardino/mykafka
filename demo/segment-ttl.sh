...
section "Now we add some text (1Ko data)"
launch "head -c 1024 /usr/share/dict/british-english | ./$PRODUCER --topic=mytopic --partition 0" "$LOG_DIR/producer.log"
...
launch "head -c 256 /usr/share/dict/british-english | ./$PRODUCER --topic=mytopic --partition 0" "$LOG_DIR/producer.log"

section "Let's check we have only 1 small files (previous 2 was out of date)"
launch "\ls -lpha $DATA_DIR/mytopic-0/*.log"
